...
import MySubmissions from '../../../components/Tables/MySubmissions'
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (user && selectedJournal) {
        try {
          setIsLoading(true);
          const res = await fetch(hostUrl + 'users/' + user.id + '/' + selectedJournal.id + '/submissions');
          if (res.ok) {
                ...
                <Tabs position="relative" variant="unstyled" isLazy>
                  ...
                  <TabPanels>
                    <TabPanel className="px-0 py-0">
                      <div>
                        <div className="bg-white min-h-[500px] h-[500px]">


                      {!loading && !isLoading ? 
                          <MySubmissions submissions={submissions} setSubmissions={setSubmissions}/>
