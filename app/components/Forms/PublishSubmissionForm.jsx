  ...
  const handleSubmit = async (e) => {
    ...
    if (submission && submission?.issue) {
      try {
        ...
        const resp = await axios.post(hostUrl + `submissions/${submission?.id}/publish`);
          ...
          <div>
            <div>
                <div className="mb-4 flex flex-col gap-1">
                  ...
                  <div>
                    <p>All publication requirements have been met.</p> <br/>
                    <p>This will be published immediately in <span className='text-blue-400 underline'>Vol. {submission?.issue?.volume} No. {submission?.issue?.number} ({submission?.issue?.year}): {submission?.issue?.title}.</span></p> 
                    <br/>
...
export default PublishSubmissionForm;
