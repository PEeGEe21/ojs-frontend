...
const UpdateContributorForm = ({
  ...
  const [isPrincipalContact, setIsPrincipalContact] = useState(contributor?.is_principal_contact == 1??false);
  ...
  const [inputs, setInputs] = useState({
    ...
    public_name: contributor?.public_name??'',
  ...
  const handleSubmit = async (e) => {
    ...
    if (handleValidation()) {
      try {
        ...
        if(submission){
          const res = await axios.patch(`${hostUrl}submissions/update-contributor/${contributor?.id}`, 
            payload
          );
